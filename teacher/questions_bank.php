...
$stmt = $db->prepare("SELECT DISTINCT subject FROM exams WHERE teacher_id = ? ORDER BY subject");
...
$query .= " GROUP BY q.question_text ORDER BY e.subject, q.question_id";
$stmt = $db->prepare($query);
$stmt->execute($params);
$questions = $stmt->fetchAll(PDO::FETCH_ASSOC);

$page_title = "Questions Bank";
$page_content = <<<HTML
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Questions Bank</h1>
    </div>
...
function copyQuestion(questionId) {
    // Add functionality to copy question to another exam
    // This will be implemented in a future update
    alert('Copy question functionality will be added soon!');
}
</script>
HTML;

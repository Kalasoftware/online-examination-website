...
// Fetch ongoing and upcoming exams for this teacher with student counts
// Update the SQL query to match admin's status logic
$stmt = $db->prepare("
    SELECT e.*, 
    ...
    FROM exams e
    WHERE e.teacher_id = ? 
    AND e.exam_date >= DATE_SUB(NOW(), INTERVAL 1 DAY)
...
foreach ($exams as $exam) {
    ...
    if ($exam['status'] === 'Ongoing') {
        $total_possible_answers = $exam['total_questions'] * $exam['registered_count'];
        $progress = $total_possible_answers > 0 ? 
            min(100, round(($exam['answered_questions'] / $total_possible_answers) * 100)) : 0;

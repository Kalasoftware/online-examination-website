...
// Get unique subjects (fixed query)
$stmt = $db->query("SELECT DISTINCT subject FROM exams ORDER BY subject");
$subjects = $stmt->fetchAll(PDO::FETCH_COLUMN);

// Initialize subject_options variable
$subject_options = '';
foreach ($subjects as $subject) {
    $selected = $subject_filter === $subject ? 'selected' : '';
    $subject_options .= "<option value=\"" . htmlspecialchars($subject) . "\" {$selected}>" . htmlspecialchars($subject) . "</option>";
}
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex space-x-3">
                            <button onclick="copyQuestion({$question['question_id']})" class="text-blue-600 hover:text-blue-800">
...
function copyQuestion(questionId) {
    if (confirm('Do you want to copy this question to another exam?')) {
        // Open modal or redirect to copy question page
